...
static ntfs_inode *__ntfs_inode_allocate(ntfs_volume *vol)
{
	...
	if (ni) {
		ni->vol = vol;
		inode_set_iversion(EXNTFS_V(ni), 1);
		inode_init_once(EXNTFS_V(ni));
...
static int ntfs_inode_sync_file_name(ntfs_inode *ni, ntfs_inode *dir_ni)
{
	...
	if (ni->flags & FILE_ATTR_REPARSE_POINT) {
		if (!(ret = ntfs_attr_lookup(AT_REPARSE_POINT, NULL,
				0, CASE_SENSITIVE, 0, NULL, 0, ctx))) {
	...
	while (!(ret = ntfs_attr_lookup(AT_FILE_NAME, NULL, 0, 0, 0, NULL, 0, ctx))) {
...
//				err = errno;
		...
		if (!ictx) {
			...
			if ((ni != index_ni) && !dir_ni
			    && (ret = ntfs_inode_close(index_ni)) && !err)
				err = ret;
			continue;
		}
		if ((ret = ntfs_index_lookup(fn, sizeof(FILE_NAME_ATTR), ictx))) {
			if (!err) {
				if (ret == -ENOENT)
					err = -EIO;
				else
			...
			if (ni != index_ni && (ret = ntfs_inode_close(index_ni)) && !err)
				err = ret;
			continue;
		}
...
static int ntfs_inode_sync_in_dir(ntfs_inode *ni, ntfs_inode *dir_ni)
{
	...
	if ((ni->mrec->flags & MFT_RECORD_IN_USE) && ni->nr_extents != -1 &&
			...
			(ret = ntfs_inode_sync_file_name(ni, dir_ni))) {
		if (!err || ret == -EIO) {
			err = ret;
			if (err != -EIO)
				err = -EBUSY;
	...
	if ((ni->mrec->flags & MFT_RECORD_IN_USE) && ni->nr_extents != -1 &&
		...
		if (na->data_size == ni->attr_list_size) {
			if ((ret = ntfs_attr_pwrite(na, 0, ni->attr_list_size,
...
sync_inode:
	...
	if (NInoTestAndClearDirty(ni)) {
		if ((ret = ntfs_mft_record_write(ni->vol, ni->mft_no, ni->mrec))) {
			if (!err || ret == -EIO) {
				err = ret;
				if (err != -EIO)
					err = -EBUSY;
...
//			}
	...
	if (ni->nr_extents > 0) {
		...
		for (i = 0; i < ni->nr_extents; ++i) {
			...
			if ((ret = ntfs_mft_record_write(eni->vol, eni->mft_no, 
						  eni->mrec))) {
				if (!err || ret == -EIO) {
					err = ret;
					if (err != -EIO)
...
//			err = errno;
	...
	if ((err = ntfs_resident_attr_record_add(ni,
...
//		err = errno;
	...
	if ((err = ntfs_attr_truncate(na, al_len))) {
//		err = errno;
